...
export default function DentalChart({
  ...
  const showUpper =
    showAllQuadrants || filteredTeeth.some((t) => t.jawType === 'UR' || t.jawType === 'UL');
  const showLower =
    showAllQuadrants || filteredTeeth.some((t) => t.jawType === 'LR' || t.jawType === 'LL');
  ...
  return (
    <svg viewBox={viewBox} width="100%" height="100%">
      {showUpper && (
        <>
          {UpperTeeths.map((t) => (
            <Tooth key={t} number={t} selectedTeeth={selectedTeeth} onToothClick={onToothClick}>
              {toothPaths[t]}
            </Tooth>
          ))}
        </>
